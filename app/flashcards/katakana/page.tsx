...
export default function KatakanaFlashcardsPage() {
  ...
  const filteredKatakana = jlptLevel === "all" ? katakana : katakana.filter((item) => item.jlpt_level === jlptLevel)

  const currentCard = filteredKatakana[currentIndex]

  const handleNext = () => {
    setFlipped(false)
    setCurrentIndex((prevIndex) => (prevIndex === filteredKatakana.length - 1 ? 0 : prevIndex + 1))
  }

  const handlePrevious = () => {
    setFlipped(false)
    setCurrentIndex((prevIndex) => (prevIndex === 0 ? filteredKatakana.length - 1 : prevIndex - 1))
  }

  const handleShuffle = () => {
    setFlipped(false)
    const randomIndex = Math.floor(Math.random() * filteredKatakana.length)
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold tracking-tight mb-4">Katakana Flashcards</h1>
      ...
      <div className="max-w-md mx-auto mb-8">
        ...
        <div className="text-center text-sm text-muted-foreground mt-2">
          Card {currentIndex + 1} of {filteredKatakana.length}
        </div>
